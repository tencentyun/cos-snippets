...
#import <QCloudCOSXML/QCloudPostSpeechRecognitionRequest.h>

@interface PostSpeechRecognitionDemo : XCTestCase <QCloudSignatureProvider, QCloudCredentailFenceQueueDelegate>

@property (nonatomic) QCloudCredentailFenceQueue* credentialFenceQueue;

@end

@implementation PostSpeechRecognitionDemo
...
- (void) testPostSpeechRecognition{
	QCloudPostSpeechRecognitionRequest * request = [QCloudPostSpeechRecognitionRequest new];
	request.bucket = @"sample-1250000000";
	request.regionName = @"COS_REGIONNAME";
	request.input = [QCloudPostSpeechRecognition new];
	// 创建任务的 Tag：SpeechRecognition;是否必传：是
	request.input.Tag = @"";
	// 待操作的对象信息;是否必传：是
	request.input.Input = [QCloudPostSpeechRecognitionInput new];
	// 操作规则;是否必传：是
	request.input.Operation = [QCloudPostSpeechRecognitionOperation new];
	// 结果输出配置;是否必传：是
	request.input.Operation.Output = [QCloudPostSpeechRecognitionOutput new];
	// 存储桶的地域;是否必传：是

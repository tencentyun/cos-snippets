...
@interface UpdateAsrQueueDemo : XCTestCase <QCloudSignatureProvider, QCloudCredentailFenceQueueDelegate>

@property (nonatomic) QCloudCredentailFenceQueue* credentialFenceQueue;

@end

@implementation UpdateAsrQueueDemo
...
- (void) testUpdateAsrQueue{
	QCloudUpdateAsrQueueRequest * request = [QCloudUpdateAsrQueueRequest new];
	request.bucket = @"sample-1250000000";
	request.regionName = @"COS_REGIONNAME";
	// 设置：queueId;
	request.queueId = @"";
	request.input = [QCloudUpdateAsrQueue new];
	// 队列名称，仅支持中文、英文、数字、_、-和*，长度不超过128;是否必传：是
	request.input.Name = @"";
	// Active 表示队列内的作业会被调度执行Paused 表示队列暂停，作业不再会被调度执行，队列内的所有作业状态维持在暂停状态，已经执行中的任务不受影响;是否必传：是
	request.input.State = @"";
	[request setFinishBlock:^(QCloudUpdateAsrQueueResponse * outputObject, NSError *error) {

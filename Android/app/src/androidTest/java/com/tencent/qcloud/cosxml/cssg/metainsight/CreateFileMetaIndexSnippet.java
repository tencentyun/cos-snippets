...
public class CreateFileMetaIndexSnippet {
    ...
    private void createFileMetaIndex() {
		// APPID
		String appid = "1253000000";
		CreateFileMetaIndexRequest request = new CreateFileMetaIndexRequest(appid);
		...
		createFileMetaIndex.datasetName = "test";
		// 设置元数据索引结果（以回调形式发送至您的回调地址，支持以 http:// 或者 https:// 开头的地址，例如： http://www.callback.com。;是否必传：否
		createFileMetaIndex.callback = "http://www.callback.com";
		CreateFileMetaIndex.File file = new CreateFileMetaIndex.File();
		createFileMetaIndex.file = file;
		...
		ciService.createFileMetaIndexAsync(request, new CosXmlResultListener() {
		    ...
		    public void onSuccess(CosXmlRequest request, CosXmlResult cosResult) {
				// result 创建元数据索引的结果
				// 详细字段请查看api文档或者SDK源码
				CreateFileMetaIndexResult result = (CreateFileMetaIndexResult) cosResult;
    ...
    @Test
    public void testCreateFileMetaIndex() {
        initService();
        createFileMetaIndex();
    }
}
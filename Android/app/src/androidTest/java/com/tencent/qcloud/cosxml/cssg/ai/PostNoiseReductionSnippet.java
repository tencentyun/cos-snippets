...
public class PostNoiseReductionSnippet {
    ...
    private void postNoiseReduction() {
		// 存储桶名称，格式为 BucketName-APPID
		String bucket = "examplebucket-1250000000";
		PostNoiseReductionRequest request = new PostNoiseReductionRequest(bucket);
		...
		postNoiseReductionInput.object = "input/demo.mp3";
		...
		// 设置存储桶的地域;是否必传：是
		postNoiseReductionOutput.region = "ap-chongqing";
		// 设置存储结果的存储桶;是否必传：是
		postNoiseReductionOutput.bucket = "test-123456789";
		// 设置输出结果的文件名;是否必传：是
		postNoiseReductionOutput.object = "output/out.wav";
		// 设置任务回调格式，JSON 或 XML，默认 XML，优先级高于队列的回调格式;是否必传：否
		postNoiseReduction.callBackFormat = "JSON";
		// 设置任务回调地址，优先级高于队列的回调地址。设置为 no 时，表示队列的回调地址不产生回调;是否必传：否
		postNoiseReduction.callBack = "http://callback.demo.com";
		...
		ciService.postNoiseReductionAsync(request, new CosXmlResultListener() {
			...
			public void onSuccess(CosXmlRequest request, CosXmlResult cosResult) {
				// result 提交任务的结果
				// 详细字段请查看api文档或者SDK源码
				PostNoiseReductionResult result = (PostNoiseReductionResult) cosResult;
    ...
    @Test
    public void testPostNoiseReduction() {
        initService();
        postNoiseReduction();
    }
}
...
public class GetAIQueueSnippet {
    ...
    private void getAIQueue() {
        // 存储桶名称，格式为 BucketName-APPID
        String bucket = "examplebucket-1250000000";
        GetAIQueueRequest request = new GetAIQueueRequest(bucket);
        request.queueIds = "p4a17eeea29334bf499b7e20e2fbfd99d";// 设置队列 ID，以“,”符号分割字符串
        request.state = "Active";// 设置Active 表示队列内的作业会被调度执行Paused 表示队列暂停，作业不再会被调度执行，队列内的所有作业状态维持在暂停状态，已经执行中的任务不受影响
        request.pageNumber = 1;// 设置第几页，默认值1
        request.pageSize = 10;// 设置每页个数，默认值10

        ciService.getAIQueueAsync(request, new CosXmlResultListener() {
            @Override
            public void onSuccess(CosXmlRequest request, CosXmlResult cosResult) {
                // result 查询AI内容识别队列的结果
                // 详细字段请查看api文档或者SDK源码
                GetAIQueueResult result = (GetAIQueueResult) cosResult;

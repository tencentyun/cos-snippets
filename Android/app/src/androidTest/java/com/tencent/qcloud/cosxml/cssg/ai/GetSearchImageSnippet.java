...
public class GetSearchImageSnippet {
    ...
    private void getSearchImage() {
        // 存储桶名称，格式为 BucketName-APPID
        String bucket = "examplebucket-1250000000";
        GetSearchImageRequest request = new GetSearchImageRequest(bucket, "folder/document.jpg");
        request.matchThreshold = 0;// 设置出参 Score 中，只有超过 MatchThreshold 值的结果才会返回。默认为0
        request.offset = 0;// 设置起始序号，默认值为0
        request.limit = 100;// 设置返回数量，默认值为10，最大值为100
        ...
        cosXmlService.getSearchImageAsync(request, new CosXmlResultListener() {
            ...
            public void onSuccess(CosXmlRequest request, CosXmlResult cosResult) {
                // result 图片搜索接口的结果
                // 详细字段请查看api文档或者SDK源码
                GetSearchImageResult result = (GetSearchImageResult) cosResult;
    ...
    @Test
    public void testGetSearchImage() {
        initService();
        getSearchImage();
    }
}
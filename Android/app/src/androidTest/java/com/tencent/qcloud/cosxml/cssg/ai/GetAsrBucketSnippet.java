...
public class GetAsrBucketSnippet {
    ...
    private void getAsrBucket() {
        DescribeSpeechBucketsRequest request = new DescribeSpeechBucketsRequest();
        request.setRegions("ap-shanghai,ap-beijing");// 设置地域信息，例如 ap-shanghai、ap-beijing，若查询多个地域以“,”分隔字符串，详情请参见 地域与域名
        request.setBucketNames("examplebucket-1250000000");// 设置存储桶名称，以“,”分隔，支持多个存储桶，精确搜索
        request.setBucketName("examplebucket");// 设置存储桶名称前缀，前缀搜索
        request.setPageNumber(1);// 设置第几页
        request.setPageSize(10);// 设置每页个数，大于0且小于等于100的整数

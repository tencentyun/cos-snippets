...
public class GetAsrQueueSnippet {
    ...
    private void getAsrQueue() {
        // 存储桶名称，格式为 BucketName-APPID
        String bucket = "examplebucket-1250000000";
        DescribeSpeechQueuesRequest request = new DescribeSpeechQueuesRequest(bucket);
        request.setQueueIds("p4a17eeea29334bf499b7e20e2fbfd99d");// 设置队列 ID，以“,”符号分割字符串
        request.setPageNumber(1);// 设置第几页，默认值1
        request.setPageSize(10);// 设置每页个数，默认值10
        request.setState("Active");// 设置Active 表示队列内的作业会被调度执行Paused 表示队列暂停，作业不再会被调度执行，队列内的所有作业状态维持在暂停状态，已经执行中的任务不受影响
